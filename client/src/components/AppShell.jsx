...
export default function AppShell({ children }) {
  ...
  useEffect(() => {
    api.getOnboardingProfile().then(profile => setLanguage(profile.primary_language || 'English')).catch(() => {});
  }, []);
  const navCopy = language === 'Hindi'
    ? ['अवलोकन', 'आकलन', 'प्रगति', 'सीखने की योजना', 'AI ट्यूटर', 'इतिहास']
    : language === 'Marathi'
      ? ['आढावा', 'मूल्यांकन', 'प्रगती', 'शिक्षण योजना', 'AI ट्यूटर', 'इतिहास']
      : ['Overview', 'Assessment', 'Progress', 'Learning plan', 'AI tutor', 'History'];
  const welcome = language === 'Hindi' ? 'वापसी पर स्वागत है' : language === 'Marathi' ? 'पुन्हा स्वागत आहे' : 'Welcome back';
  const signOutLabel = language === 'Hindi' ? 'साइन आउट' : language === 'Marathi' ? 'बाहेर पडा' : 'Sign out';
  return (
    <div className="min-h-screen bg-[#f7f8fa] text-slate-900">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 space-y-1 px-3 py-4">
          ...
          {links.map(([to, , Icon], index) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${isActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}><Icon size={18} />{navCopy[index]}</NavLink>)}
        </nav>
        <div className="border-t border-slate-100 p-4">
          ...
          <button onClick={signOut} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-rose-50 hover:text-rose-600"><LogOut size={16} />{signOutLabel}</button>
        </div>
      </aside>
      {open && <button aria-label="Close menu" className="fixed inset-0 z-30 bg-slate-900/20 lg:hidden" onClick={() => setOpen(false)} />}
      <main className="min-h-screen lg:pl-64"><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur lg:px-8"><button className="rounded-lg p-2 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(true)}><Menu size={21} /></button><div className="ml-auto text-sm text-slate-500">{welcome}, <span className="font-semibold text-slate-800">{user?.email?.split('@')[0]}</span></div></header><div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">{children}</div></main>
    </div>
  );
}
